...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // frame's clamp range.
      // Guard against ResizeObserver feedback: only re-render when the
      // observed box actually changed size. Rounding to whole px absorbs
      // sub-pixel jitter (scrollbar toggles, layout settling) that would
      // otherwise refire _render across many instances and peg the thread.
      this._roW = -1; this._roH = -1;
      this._ro = new ResizeObserver(() => {
        const w = Math.round(this.clientWidth), h = Math.round(this.clientHeight);
        if (w === this._roW && h === this._roH) return;
